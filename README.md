...
A compressed file with the binaries is created in `<ValheimInfiniteFuel>\Packages`ready for upload to ThunderStore.
...
A Unity project is included in this repository under `<ValheimInfiniteFuel>\JotunnModUnity`.
...
4. Compile the project. This copies all assemblies into `<ValheimInfiniteFuel>\JotunnModUnity\Assets\Assemblies`. Don't open Unity yet before this step, it will remove assembly references.
...
1. Copy all `assembly_*.dll` from `<ValheimDir>\valheim_Data\Managed` into `<ValheimInfiniteFuel>\JotunnModUnity\Assets\Assemblies`. <br />
